...
srtsubs_path = xbmc.translatePath('special://temp/kidamom.Bulgarian.srt')
...
def CATEGORIES():
       ...
       addDir('Видео Облак', baseurl + '/cloud', 2, fol1)
...
def SHOW(url):
       ...
       if 'cloud' in url:
           try:
               match1 = re.compile('<h2 id="movie-name">(.+?)</h2>').findall(data)
               for name in match1:
                   title = name
           except:
               title = 'Kidamom филм'
           match2 = re.compile('<source src="(.+?)" .{20,50}/>').findall(data)
           match3 = re.compile('<track .{50,80} src="(.+?)" .{10,30}/>').findall(data)
           for video in match2:
               vd = video+'|User-Agent='+urllib.quote_plus(UA)+'&Referer='+url
               for subtitle in match3:
                   vd = video+'|User-Agent='+urllib.quote_plus(UA)+'&Referer='+url+'@'+baseurl+subtitle
               addLink(title,vd,4,'','')
...
def PLAY(url):
       if not '@' in url:
           ...
           stream, srtsubtitles = url.split("@")
           #Изтегляме и зареждаме субтитрите ако има външни такива
